...
class AegisQuantLabGUI:
    ...
    def _build_ctk_layout(self):
        ...
        ctk.CTkButton(left, text="Generate Weight Matrix",
                       command=self._on_generate_weights, fg_color="#8e44ad", hover_color="#6c3483").pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(left, text="Run Consensus Backtest",
                       command=self._on_consensus_backtest, fg_color="#2980b9", hover_color="#1a5276").pack(fill="x", padx=10, pady=5)
    ...
    def _build_tk_layout(self):
        ...
        tk.Button(left, text="Download / Update Data", command=self._on_download,
                  bg="#e67e22", fg="white", font=("Helvetica", 11, "bold")).pack(fill="x", padx=10, pady=5)
        tk.Button(left, text="Generate Weight Matrix", command=self._on_generate_weights,
                  bg="#8e44ad", fg="white", font=("Helvetica", 11, "bold")).pack(fill="x", padx=10, pady=5)
        tk.Button(left, text="Run Consensus Backtest", command=self._on_consensus_backtest,
                  bg="#2980b9", fg="white", font=("Helvetica", 11, "bold")).pack(fill="x", padx=10, pady=5)
    ...
    # ──────────────────────────────────────────
    # Generate Weight Matrix action
    # ──────────────────────────────────────────

    def _on_generate_weights(self):
        if self._running:
            self._log("Another task is already running. Please wait.")
            return
        self._running = True
        self._set_status("Generating weight matrix …")
        threading.Thread(target=self._weight_matrix_worker, daemon=True).start()

    def _weight_matrix_worker(self):
        try:
            import data_loader
            import generate_weight_matrix
            ...
            # Load data
            self._log(f"\n{'=' * 60}")
            self._log("WEIGHT MATRIX GENERATION")
            self._log(f"{'=' * 60}")
            self._log(f"Loading {len(symbols)} symbols ({tf}) …")
            ...
            self._log(f"Loaded {len(raw_data)} symbols. Computing weights …")
            self._log(f"Bybit roundtrip fee: {config.ROUNDTRIP_FEE*100:.3f}%")
            self._log(f"Forward evaluation window: {config.FORWARD_BARS} bars")

            # Generate weights
            library = generate_weight_matrix.generate_weight_library(raw_data, progress_cb=self._log)
            path = generate_weight_matrix.save_weight_library(library)

            # Summary
            summary_df = generate_weight_matrix.get_symbol_summary(library)
            if not summary_df.empty:
                self._log(f"\n{'─' * 50}")
                self._log(f"  WEIGHT MATRIX SUMMARY: {len(library)} symbols")
                self._log(f"{'─' * 50}")
                for _, row in summary_df.head(20).iterrows():
                    self._log(
                        f"  {row['symbol']:12s} | Best: {row['best_indicator']:30s} | "
                        f"Weight={row['best_weight']:.4f} | WR={row['best_win_rate']:.1f}% | PF={row['best_pf']:.2f}"
                    )

            self._log(f"\nWeight matrix saved: {path}")
            self._set_status("Weight matrix generated")

        except Exception as e:
            self._log(f"\nERROR: {e}")
            logger.exception("Weight matrix generation failed")
            self._set_status("Weight generation failed")
        finally:
            self._running = False

    # ──────────────────────────────────────────
    # Consensus Backtest action
    # ──────────────────────────────────────────

    def _on_consensus_backtest(self):
        if self._running:
            self._log("Another task is already running. Please wait.")
            return
        self._running = True
        self._set_status("Running consensus backtest …")
        threading.Thread(target=self._consensus_worker, daemon=True).start()
    ...
    def _consensus_worker(self):
        try:
            ...
            # Load weight library
            library = generate_weight_matrix.load_weight_library()
            if not library:
                self._log("No weight library found. Please generate weights first.")
                self._set_status("No weights")
                return

            # Load data
            self._log(f"\n{'=' * 60}")
            self._log("CONSENSUS VOTING BACKTEST")
            self._log(f"{'=' * 60}")
            self._log(f"Top indicators per symbol: {config.CONSENSUS_TOP_N_INDICATORS}")
            self._log(f"Min agreement for trade: {config.CONSENSUS_MIN_AGREEMENT:.0%}")
            self._log(f"Dissonance block threshold: {config.CONSENSUS_DISSONANCE_THRESHOLD}")
            self._log(f"Capital: ${config.DEFAULT_CAPITAL:.0f} USDT, Risk/trade: {config.RISK_PER_TRADE_PCT:.0%}")

            raw_data = data_loader.load_all_local(symbols, tf, start, end)
            if not raw_data:
                self._log("No cached data. Please download first.")
                return

            # Run consensus backtest
            results = consensus_engine.backtest_consensus_batch(raw_data, library, progress_cb=self._log)

            # Display results
            self._log(f"\n{'─' * 60}")
            self._log(f"  CONSENSUS BACKTEST RESULTS ({len(results)} symbols)")
            self._log(f"{'─' * 60}")

            total_pnl = 0.0
            for sym, r in sorted(results.items(), key=lambda x: x[1].get("total_pnl_pct", 0), reverse=True):
                if "error" in r:
                    continue
                total_pnl += r.get("total_pnl_pct", 0)
                self._log(
                    f"  {sym:12s} | Trades={r.get('total_trades', 0):4d} | "
                    f"WR={r.get('win_rate_pct', 0):5.1f}% | "
                    f"PnL={r.get('total_pnl_pct', 0):+7.2f}% | "
                    f"PF={r.get('profit_factor', 0):5.2f} | "
                    f"SR={r.get('sharpe_ratio', 0):5.2f}"
                )

            self._log(f"\n  TOTAL PORTFOLIO PnL: {total_pnl:+.2f}%")
            self._log(f"  On ${config.DEFAULT_CAPITAL:.0f} USDT: ${total_pnl/100*config.DEFAULT_CAPITAL:+.2f} USDT")
            self._set_status("Consensus backtest complete")

        except Exception as e:
            self._log(f"\nERROR: {e}")
            logger.exception("Consensus backtest failed")
            self._set_status("Consensus BT failed")
        finally:
            self._running = False
